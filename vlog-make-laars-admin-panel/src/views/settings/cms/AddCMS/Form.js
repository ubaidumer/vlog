...
const Form = ({ className, handleModalClose, ...rest }) => {
  ...
  const [dutchError, setDutchError] = useState(false);
  ...
  }, [formState.values, isProfilePictureUploaded, dutch, english]);
  ...
  const handleSubmit = event => {
    ...
    if (formState.isValid && dutch.length > 4 && value != '') {
      (async () => {
        setIsSubmitting(true);
        // Prepare Headers and Client
        axios
          .post(`${process.env.REACT_APP_CMS_URL}`, body, {
            headers: { authorization: `bearer ${token}` }
          })
          .then(response => {
            enqueueSnackbar('Record is succesvol aangemaakt.', {
              variant: 'success'
            });
    ...
    <form onSubmit={handleSubmit} className={classes.root}>
      <Grid container>
        ...
        <Grid item md={9} xs={12}>
          ...
          {dutchError ? (
            <div style={{ color: '#ff3535',fontSize:'13px' }}>Lengte is te kort.</div>
          ) : (
            <div></div>
          )}
        </Grid>

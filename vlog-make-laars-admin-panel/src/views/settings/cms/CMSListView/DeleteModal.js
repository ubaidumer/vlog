...
const DeleteModal = props => {
  ...
  const { selectedClients, openDeleteModal, setOpenDeleteModal } = props;
  ...
  const deleteUsers = () => {
    const query = gql`
      mutation {
        removeMultipleUsers(
          isDeleted: 1
          userIdList: ${JSON.stringify(selectedClients)}
        ) {
          _id
        }
      }
  ...
  return (
    <div>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Weet u zeker dat u klantrecord(s) wilt verwijderen?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button

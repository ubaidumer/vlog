...
import InvoiceDetailsView from './emailTemp/index';
...
import BicycleRentingDetailsView from '../editEmail';
...
const EmailTemplates = ({
  ...
  let handleDel = (id, data) => {
    setId(id);
  };

  let handleDeleteTemplate = () => {
    Axios.delete(`${process.env.REACT_APP_TEMPLATE_URL}/${id}`)
      .then(data => {
        enqueueSnackbar('E-mailtemplate(s) zijn succesvol verwijderd.', {
          variant: 'error'
        });
      ...
      .catch(error => {
        enqueueSnackbar('Er is een fout opgetreden.', {
          variant: 'error'
        });
      });
  };
      ...
      <PerfectScrollbar>
        <Box minWidth={1200}>
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className={classes.firstTd}>Template</TableCell>
                ...
                if (all) {
                  ...
                  return (
                    <TableRow hover key={email.id} selected={isemailSelected}>
                      {/* <TableCell padding="checkbox">
                        <Checkbox
                          onClick={() => handleDel(email.id, email)}
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                Wilt u echt e-mailsjabloonrecord(s) verwijderen?
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button
              ...
              <Button onClick={handleDeleteTemplate} color="primary" autoFocus>
                Delete
              </Button>
            </DialogActions>
          </Dialog>
